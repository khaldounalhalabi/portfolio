...
- **Resume cache-busting**: each regeneration uploads the PDF under a unique timestamped object path, deletes the previous PDFs, and writes the fresh public URL into the `resume_link` site setting (`ResumeService` in `services/`). The portfolio nav (`components/portfolio/site-layout.tsx`) links to `resume_link` directly (falling back to `/resume.pdf`), so a new URL each time avoids Supabase Storage/browser caching serving a stale resume.
